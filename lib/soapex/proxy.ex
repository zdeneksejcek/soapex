      ...
      Enum.each(@t_wsdl, fn {service_name, service_value} ->
        Enum.each(service_value, fn {port_name, port_value} ->
          ...
          Enum.each(port_value.operations, fn {op_name, op_value} ->
            params = op_value.input_message.parts |> Enum.map(fn p -> Macro.var(:"#{Macro.underscore(p.name)}", __MODULE__) end)
            params_list = op_value.input_message.parts |> Enum.map(fn p -> {"#{p.name}", Macro.var(:"#{Macro.underscore(p.name)}", __MODULE__)} end)
            ...
            def unquote(:"#{Macro.underscore(op_name)}")(
                  {unquote(Macro.escape(service_name)), unquote(Macro.escape(port_name))} = port_path,
